...
- **ToolSimulator wrapper** adds per-tool overlays for lessons 11-16: Claude lavender+serif, Cursor IDE-dark+green, Gemini blue→violet gradient with 📎 Attach picture menu, Codex GitHub gray-green+mono, OpenCode light+teal+🔓, Ollama mono terminal. Persistent `✨ Pretend mode — not real internet` chip in every overlay's corner. Collapsed by default behind a "Try the pretend helper!" button to avoid screen overload.
