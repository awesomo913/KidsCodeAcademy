...
- Lesson, question, and answer narration is pre-baked with Piper and shipped as OGG audio in a compact bundle — runtime is TTS-free for authored content.
- 24 mascot animation frames are procedurally generated by `gen_mascot.py` (Pillow primitives) — zero external art.
- The 60 lessons span 10 chapters, with interactive games, six sandbox AI helpers, chess breaks, and a Lesson 60 game-making capstone.
- localStorage holds browser progress; the desktop bridge mirrors state to `%APPDATA%\KidsCodeAcademy\state.json` and stores projects in `%APPDATA%\KidsCodeAcademy\kid_projects\`.
...
- Parent Corner can now either continue at a chosen lesson or mark every lesson through it complete.
- Every lesson has three math-minute questions drawn from 26 age-appropriate skill categories; all 180 prompts are unique and have pre-baked audio.
- Lesson-wide repeated distractors are capped and checked during the build.
- The one-file EXE stores thousands of small narration clips in one audio bundle, reducing tested cold startup from about 69 seconds to about 18 seconds.
- Chess, 60 lesson files, all referenced math audio, packaged persistence, and desktop startup were verified on Windows.
...
1. **Fix 1 (Bytey art) — PARTIAL ATTEMPT 2026-05-14.** Chrome MCP path connected to user's logged-in Gemini. **Hero #1 (idle) generated successfully** and saved as a reference at `assets/mascot_heroes/idle.png` — cute light-blue chubby robot with lightning chest emblem, dot eyes + cheek blush, `<b>` shoes, gentle smile. PNG 397 KB at 1024×1024.
   - **Heroes #2-4 wedged.** After the first successful generation, every subsequent prompt in the same Chrome MCP session produced a blob URL in the DOM but the `<img>` element never loaded the bytes (`naturalWidth=0` indefinitely). Tried: fresh chat, page reload, scroll-into-view, image.src reassign, Gemini's own "Download full size" button — all wedged. Could be Chrome MCP middleware blocking subsequent blob loads, Gemini lazy-loading hidden behind an intersection observer Chrome MCP doesn't trigger, or session-level rate limit.
   - **Next session pickup:** quit Chrome entirely, reconnect extension fresh, generate one hero per fresh Chrome session if needed. OR: use the existing `assets/mascot_heroes/idle.png` as the single source of truth + procedurally tween in `gen_mascot.py` (head bob → idle 8, rotate-arm-up sequence → wave 6, scale+jump → cheer 6, head tilt → think 4). Animation contract preserved: `assets/mascot/{idle_01..08, wave_01..06, cheer_01..06, think_01..04}.png` exact filenames + counts.
   - **Current ship:** v0.7.11 + v0.7.12 (this commit) still uses original procedural Pillow Bytey. Real-art swap deferred.
